...
# CMD="python ${RLRECONSTRUCT_PY_DIR}/reward_learning/train_v4.py \
CMD="python ${RLRECONSTRUCT_PY_DIR}/reward_learning/input_pipeline_performance.py \
    --data-path $DATA_PATH \
    ...
    --tf.cpu_train_queue_capacity $((1024 * 4)) \
    --tf.cpu_test_queue_capacity $((1024 * 4)) \
    --tf.cpu_train_queue_threads 4 \
    --tf.cpu_test_queue_threads 2 \
    --tf.cpu_train_queue_processes 4 \
    --tf.cpu_test_queue_processes 2 \
