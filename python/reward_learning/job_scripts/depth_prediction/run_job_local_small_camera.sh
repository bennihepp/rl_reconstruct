...
export MODEL_PATH=$HOME/reward_learning/models/depth_prediction_small_camera/$JOB_ID
export LOG_PATH=$HOME/reward_learning/logs/depth_prediction_small_camera/$JOB_ID

mkdir -p $LOG_PATH
mkdir -p $MODEL_PATH

bash run_job_${JOB_ID}.sh --verbose-model ${@:3} 2>&1 | tee $LOG_PATH/stdout.log
