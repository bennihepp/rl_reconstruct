...
#SCENE_ID=20
#SCENE_ID=30
OUTPUT_FOLDER=outputs_scenes/output_${CAMERA_NAME}_video
...
function run_evaluation {
  ...
  if [ $POLICY_MODE == "prediction" ]; then
    ...
    if [ -n "$5" ]; then
      MODEL_PATH=$HOME/rl_reconstruct_models/reward_prediction_small_camera_big_scenes/$5/${MODEL_NUM}${MODEL_POSTFIX}/
      CONFIG=$HOME/rl_reconstruct/python/reward_learning/job_scripts/$5/config_${MODEL_NUM}.yaml
    else
      MODEL_PATH=$HOME/rl_reconstruct_models/reward_prediction_small_camera_big_scenes/${MODEL_NUM}${MODEL_POSTFIX}/
...
run_evaluation prediction 100 model.ckpt-93750 "" 16x16x16_0-1-2-3
done
