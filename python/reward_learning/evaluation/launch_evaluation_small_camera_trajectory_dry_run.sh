...
#OUTPUT_FOLDER=outputs_scenes/output_${CAMERA_NAME}_trajectory
...
function run_evaluation {
  ...
  # Evaluation script
  tmux send-keys -t $SESSION:4 "python $HOME/rl_reconstruct/python/reward_learning/evaluate_reward_trajectory.py \\" C-m \
...
METHOD=model_0040_prediction

name="output${SCENE_ID}_${METHOD}"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

tmux attach -t $SESSION:4

exit

#name="output${SCENE_ID}_model_0040_prediction"
#run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

#name="output${SCENE_ID}_oracle"
#run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
...
function convert_noise_trajectories {
  local METHOD=$1

  name="output${SCENE_ID}_stereo_sgbm_${METHOD}_stereo_sgbm_0.50"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

  name="output${SCENE_ID}_stereo_sgbm_${METHOD}_stereo_sgbm_0.20"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

  #name="output${SCENE_ID}_stereo_sgbm_0.2_${METHOD}_stereo_bm_0.20"
  #run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

  name="output${SCENE_ID}_${METHOD}_noise_0.05_0.00"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
  name="output${SCENE_ID}_${METHOD}_noise_0.05_0.05"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
  name="output${SCENE_ID}_${METHOD}_noise_0.05_0.10"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

  name="output${SCENE_ID}_${METHOD}_noise_0.10_0.00"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
  name="output${SCENE_ID}_${METHOD}_noise_0.10_0.05"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
  name="output${SCENE_ID}_${METHOD}_noise_0.10_0.10"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

  name="output${SCENE_ID}_${METHOD}_noise_0.20_0.00"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
  name="output${SCENE_ID}_${METHOD}_noise_0.20_0.05"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
  name="output${SCENE_ID}_${METHOD}_noise_0.20_0.10"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

  name="output${SCENE_ID}_${METHOD}_noise_0.50_0.25"
  run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
}

convert_noise_trajectories oracle
convert_noise_trajectories model_0040_prediction
convert_noise_trajectories rpg_ig_ProximityCountIg
...
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.05_0.00"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.05_0.05"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.05_0.10"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.10_0.00"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.10_0.05"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.10_0.10"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.20_0.00"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.20_0.05"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.20_0.10"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj

name="outputbuildings3_large_steps_200_model_0040_prediction_noise_0.50_0.25"
run_evaluation $INPUT_FOLDER/${name} $OUTPUT_FOLDER/${name}_traj
