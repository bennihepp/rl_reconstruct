...
while true; do
    ...
    echo "Exit code from worker: $worker_exit_code"
    if [ "$worker_exit_code" -eq 0 ]; then
    	echo "Worker finished without error. Shutting down ROS and Unreal."
    ...
    echo "Restarting worker process (including ROS and Unreal)"
