...
{ 
  ...
  , score_bbox_min_(-std::numeric_limits<double>::max(), -std::numeric_limits<double>::max(), -std::numeric_limits<double>::max())
  , score_bbox_max_(+std::numeric_limits<double>::max(), +std::numeric_limits<double>::max(), +std::numeric_limits<double>::max())
  {
  }
  ...
  void CSCOPE::calculateIgsOnRay( RayCaster::Ray& ray, std::vector< boost::shared_ptr< InformationGain<TREE_TYPE> > >& ig_set, RayCastSettings& setting )
  {
    ...
    if( found_endpoint )
    {
      ...
      if( this->link_.octree->coordToKeyChecked(end_point, end_key) )
      {
        ...
        const Eigen::Vector3d position(end_point.x(), end_point.y(), end_point.z());
        const bool in_score_bbox = (position.array() >= score_bbox_min_.array()).all() && (position.array() <= score_bbox_max_.array()).all();
        if (in_score_bbox) {
          BOOST_FOREACH( typename InformationGain<TREE_TYPE>::Ptr& ig, ig_set )
          {
