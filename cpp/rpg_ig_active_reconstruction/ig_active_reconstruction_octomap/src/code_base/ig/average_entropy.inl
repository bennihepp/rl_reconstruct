...
//    std::cout << "End point measurement: node=" << (node) << ", occ=" << occ << ", ent=" << ent << std::endl;
    // only include rays that hit an occupied!
    if( utils_.isOccupied(occ) )
    {
//      std::cout << "Endpoint is occupied. Increasing IG by " << current_ray_entropy_
//                << " and voxel count by " << voxel_count_ << std::endl;
...
//    if (node != NULL) {
//      std::cout << "Voxel measurement: node=" << (node) << ", occ=" << occ << ", ent=" << ent << std::endl;
//    }
